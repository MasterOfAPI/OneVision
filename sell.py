...
# 비트코인 upper 확인
    for _ in range(200):
        ...
        median_ask_price = df_asks["ask_price"].median()
        median_ask_size = df_asks["ask_size"].median()
        max_ask_price = df_asks["ask_price"].max()
        max_ask_size = df_asks["ask_size"].max()

        print("대상 :", boss_crypto_name)
        print("매도 가격 중간 값 :", median_ask_price)
        print("매도 잔량 중간 값 :", median_ask_size)
        print("매도 가격 최대 값 :", max_ask_price)
        print("매도 잔량 최대 값 :", max_ask_size)

        # 보유 물량 확인
        my_ask_size = upbit.get_balance(boss_crypto_name)
        print("보유 물량 :", my_ask_size)

        # 매도 API 호출 (※주의항목※)
        if current_upper < -0.05 and my_ask_size > 0:
            print("매도 조건 충족: 매도를 진행합니다.")
            my_ask_price = max_ask_price * 0.9
            my_ask_size = upbit.get_balance(boss_crypto_name)
            print(upbit.sell_limit_order(boss_crypto_name, max_ask_price, my_ask_size))
        elif my_ask_size == 0:
            print("\n매도 조건 미충족: 소유한 물량이 없습니다.")
            # 아무 것도 하지 않음
        else:
            print("\n매도 조건 미충족: 조건 값을 재조회합니다.")
            # 아무 것도 하지 않음